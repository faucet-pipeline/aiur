...
function generatePageTree({ pages }, referenceDir, { root, parent } = {}) {
	let entries = new Map();
	let topLevel = !root;
	if(topLevel) {
	...
	Object.entries(pages).forEach(([slug, descriptor]) => {
...
function generatePage(slug, descriptor, referenceDir, root) {
	...
	let page = new Page(slug, filepath, children &&
			generatePageTree({ pages: children }, referenceDir, { root, parent: slug }));
	root.index(page);
	return page;
}

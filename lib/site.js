...
let { resolvePath } = require("faucet-pipeline-core/lib/util");
let { abort, reportFileStatus } = require("faucet-pipeline-core/lib/util");
let { mkdirp } = require("mkdirp");
let { readFile, writeFile } = require("node:fs/promises");
...
module.exports = class Site {
	constructor({ source, target }, assetManager) {
		...
		Object.assign(this, {
			...
			baseURI: assetManager.manifest.baseURI,
	...
	async generate(filepaths) {
		if(filepaths) {
			// only copy the files on the first run
			this.copyFile("prismjs/themes/prism.css", `${this.target}/prism.css`);
			this.copyFile("aiur/lib/client/style.css", `${this.target}/style-aiur.css`);
			this.copyFile("aiur/lib/client/index.js", `${this.target}/script-aiur.css`);
		}
	...
	async copyFile(source, destination) {
		let content = await readFile(this.assetManager.resolvePath(source));
		this.assetManager.writeFile(destination, content);
	}
};

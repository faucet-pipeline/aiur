...
function makeRunner(config, assetManager) {
	...
	return filepaths => {
		...
		if(source.endsWith(".json")) { // eslint-disable-next-line no-var
			var pages = readFile(source, "utf8").then(JSON.parse);
		} else {
			let filepath = path.resolve(referenceDir, source);
			pages = Promise.resolve(require(filepath));
		}

		pages.
			then(pageDescriptors => {
				pages = normalize(pageDescriptors, config, referenceDir);

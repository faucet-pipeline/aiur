...
function makeRunner(config, assetManager) {
	...
	return filepaths => {
		...
		return readFile(source).
			then(json => {
				let pages = normalize(JSON.parse(json), config,
...
function normalize(pageDescriptors, config, resolvePath) {
	return Object.keys(pageDescriptors).reduce((memo, slug) => {
		...
		if(children) {
			descriptor.children = normalize(children, config, resolvePath);
		}

		let page = new Page(descriptor, config, resolvePath);
